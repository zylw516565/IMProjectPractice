...
void LoginServerClient::onConnection(const TcpConnectionPtr& conn)
{
	if (conn->connected())
	{
		LOGD("connect server: %s success", conn->peerAddress().toIpPort().c_str());
...
{
	LOGD("connection: %s is down", conn->peerAddress().toIpPort().c_str());
}

void LoginServerClient::onWriteComplete(const TcpConnectionPtr& conn)
{
	LOGD("connection: %s is writecomplete", conn->peerAddress().toIpPort().c_str());
}
